...
-- Faces policies
CREATE POLICY "Users can view their own faces" ON faces FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can manage their own faces" ON faces FOR ALL USING (auth.uid() = user_id);
...
CREATE POLICY "Teachers and admins can manage classes" ON classes FOR ALL USING (
    EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('guru', 'admin'))
);

-- Class students policies
CREATE POLICY "Students can view their own enrollments" ON class_students FOR SELECT USING (auth.uid() = student_id);
CREATE POLICY "Teachers and admins can manage enrollments" ON class_students FOR ALL USING (
    EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('guru', 'admin'))
);

-- Attendance policies
CREATE POLICY "Students can view their own attendance" ON attendance FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Teachers can view their class attendance" ON attendance FOR SELECT USING (
    EXISTS (
        SELECT 1 FROM class_students cs
        JOIN classes c ON cs.class_id = c.id
        WHERE cs.student_id = attendance.user_id
...
-- Grades policies
CREATE POLICY "Students can view their own grades" ON grades FOR SELECT USING (auth.uid() = student_id);
CREATE POLICY "Teachers can view grades they created" ON grades FOR SELECT USING (auth.uid() = teacher_id);
CREATE POLICY "Teachers and admins can manage grades" ON grades FOR ALL USING (
    EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('guru', 'admin'))
);

-- Exams policies
CREATE POLICY "Students can view published exams" ON exams FOR SELECT USING (
...
CREATE POLICY "Students can view their own results" ON exam_results FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Teachers and admins can view all results" ON exam_results FOR SELECT USING (
    EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('guru', 'admin'))
);

-- Pengumuman policies
CREATE POLICY "Everyone can view active announcements" ON pengumuman FOR SELECT USING (is_active = true);
